...
void RecursiveDFS(List* firstElem) {
    if ((firstElem->getNode(firstElem)) != nullptr && (firstElem->getNode(firstElem))->getMark() == false) {
        (firstElem->getNode(firstElem))->setMark(true);
        for (int i = 1; i <= firstElem->length(); i++) {
            firstElem->next();
            RecursiveDFS(firstElem);
        }
    }
}

void IterativeDFS(List* firstElem)
{
	Stack s;
	
	s.push(*firstElem->getNode(firstElem));
	while(!s.isEmpty())
	{
		Node v=s.pop();
		if(v.getMark()==false)
...
int main() {
    ...
    cout << "Please give the source vertex for Depth First Search: ";
    cin >> elem;
    
    IterativeDFS(arrList[elem].getElem());
